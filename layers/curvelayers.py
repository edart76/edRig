...
class VariableFkOp(LayerOp):
    ...
    inputs = {
        "jointCurve" : {
            "value" : None,
            "datatype" : "1D",
            "desc" : "curve on which to create variableFk"
        },
        "ctrlCount" : {
            "value" : 6,
            "datatype" : "int",
            "desc" : "how many controls to create"
        }
    }
    ...
    outputs = {
        "jc" : {
            "value" : None,
            "datatype" : "1D",
            "desc" : "output curve from variableFk"
        }
    }
    def __init__(self, name="VarFkOp"):
        super(VariableFkOp, self).__init__(name)


class BezierOp(LayerOp):
    """basis of all better ik splines -
    spaceswitched controls for curves"""

    inputs = {
        "startParent" : {
            "datatype" : "0D",
            "desc" : "parent space for the start control"
            },
        "endParent" : {
            "datatype" : "0D",
            "desc" : "parent space for the end control"
            },
        "curve" : {
            "datatype" : "1D",
            "desc" : "curve to pass through bezier controls"
            },
        "spaceSwitch" : {
            "datatype" : "spaceSwitch",
            "desc" : "spaceSwitch to connect on controls"
            }
        }
    ...
    outputs = {
        "jointCurve" : {
            "datatype" : "1D",
            "desc" : "output curve of bezier controls"
        }
    }

    def __init__(self, name="BezOp"):
        super(BezierOp, self).__init__(name)
...
class CurveFromPointsOp(LayerOp):
    ...
    inputs = {
        "points" : {
	        "valueList" : [],
	        "datatype" : "0D",
	        "desc" : "parent points used to drive curve"
        }
    }
