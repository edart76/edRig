#!/usr/bin/python
from edRig.tilepile.jchan2.base.commands import PropertyChangedCmd
from edRig.tilepile.jchan2.base.model import NodeModel
from edRig.tilepile.jchan2.base.port import Port
from edRig.tilepile.jchan2.widgets.node_backdrop import BackdropNodeItem
from edRig.tilepile.jchan2.widgets.node_base import NodeItem
...
class NodeObject(object):
	...
	__identifier__ = 'edRig.tilepile.jchan2.nodes'
	...
	def graph(self):
		...
		Returns:
			edRig.tilepile.jchan2.NodeGraph: node graph.
		"""
		return self._graph
	...
	def update(self):
		...
		print ""
		print "tile model is {}".format(settings)
...
class Node(NodeObject):
	...
	def add_input(self, name='input', multi_input=False, display_name=True):
		...
		Returns:
			edRig.tilepile.jchan2.Port: the created port object.
		"""
	...
	def add_combo_menu(self, name='', label='', items=None, tooltip=None):
		...
		widget = self.view.add_combo_menu(name, label, items, tooltip=tooltip)
		widget.value_changed.connect(lambda k, v: self._on_widget_changed(k, v))

	def add_text_input(self, name='', label='', text='', tooltip=None):
		"""
		...
		widget = self.view.add_text_input(name, label, text, tooltip=tooltip)
		widget.value_changed.connect(lambda k, v: self._on_widget_changed(k, v))

	def add_int_spinbox(self, name='', label='', value=0,
						min=None, max=None, tooltip=""):
		self.create_property(name, value)
		widget = self.view.add_intSpinbox(name=name, label=label, value=value,
										  min=min, max=max, tooltip=tooltip)
		widget.value_changed.connect(lambda k, v: self._on_widget_changed(k, v))

	def add_float_spinbox(self, name='', label='', value=0,
						min=None, max=None, tooltip=""):
		self.create_property(name, value)
		widget = self.view.add_floatSpinbox(name=name, label=label, value=value,
											min=min, max=max, tooltip=tooltip)
		widget.value_changed.connect(lambda k, v: self._on_widget_changed(k, v))
	...
	def add_checkbox(self, name='', label='', text='', state=False, tooltip=None):
		...
		widget = self.view.add_checkbox(name, label, text, state, tooltip=tooltip)
		widget.value_changed.connect(lambda k, v: self._on_widget_changed(k, v))
	...
	def input(self, index):
		...
		Returns:
			edRig.tilepile.jchan2.Port: port object.
		"""
	...
	def set_input(self, index, port):
		...
		Args:
			...
			port (edRig.tilepile.jchan2.Port): port object.
		"""
	...
	def clear_io(self):
		...
		self.view.delete_io()
		print "cleared tile io"
		self.update()
		#self.view.delete_io()

	def clear_outputs(self):
		"""crude for now, but it's enough"""
