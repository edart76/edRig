...
from edRig.tesserae.jchan2.widgets.constants import (IN_PORT, OUT_PORT,
                                                     NODE_ICON_SIZE, ICON_NODE_BASE,
                                                     NODE_SEL_COLOR, NODE_SEL_BORDER_COLOR,
                                                     Z_VAL_NODE, Z_VAL_NODE_WIDGET)

from edRig.tesserae.jchan2.widgets.graphics_widgets import (NodeBaseWidget,
                                                            NodeComboBox,
                                                            NodeLineEdit,
                                                            NodeCheckBox,
                                                            NodeFloatSpinbox,
                                                            NodeIntSpinbox)
from edRig.tesserae.jchan2.widgets.node_abstract import AbstractNodeItem
from edRig.tesserae.jchan2.widgets.port import PortItem

#
# class NodeLabel(QGraphicsTextItem):
#     """subclassing for renaming nodes on doubleclick"""
#     NodeNameChange = QtCore.Signal(str)
#
#     def __init__(self, value="", parent=None):
#         super(NodeLabel, self).__init__(parent)
#         self.setPlainText(value)
#
#         # self.renameField = QLineEdit(parent=super(NodeLabel, self))
#         # self.renameField = QLineEdit(self)
#         # self.renameField.setParent(self)
#
#     #
#     # def mouseDoubleClickEvent(self, event):
#     #     print "mouse doubleClick"
#     #     self.begin_name_edit()
#     #     super(NodeLabel, self).mouseDoublePressEvent(event)
#
#     def begin_name_edit(self):
#         """brings up QLineEdit to rename node
#         then passes that to the model to check name is valid"""
#         print "beginning name edit"
#         self.renameField.setText(self.toPlainText())
#         self.renameField.show()
#
#
#     def pass_name(self):
#         """called when return is pressed on renameField"""
#         self.NodeNameChange.emit(self.renameField.text())
#         self.renameField.close()
#
#
#     def end_name_edit(self, name):
#         """value returned from model, ready to be drawn
#         node is already renamed by this point
#         to be connected to a higher signal"""
#         self.setText(name)
...
class NodeItem(AbstractNodeItem):
	...
	def __init__(self, name='node', parent=None):
		...
		#self._text_item = NodeLabel(self.name, self)
		#self._rename_widget = RenameWidget(self)
	...
	def mouseDoubleClickEvent(self, event):
		"""implemented for node renaming, a la maya"""
		# add check for double click being performed on name label
		#self._begin_name_edit()
		#super(NodeItem, self).mouseDoublePressEvent(event) # doesn't exist?
		pass
	...
	def post_init(self, viewer=None, pos=None):
		...
		Args:
			viewer (edRig.tilepile.jchan2.widgets.viewer.NodeViewer): not used
	...
	def add_intSpinbox(self, name="", label="", tooltip="",
	                   value=0, min=None, max=None):
		widget = NodeIntSpinbox(self, name, label, value, min, max)
		widget.setToolTip(tooltip)
		self.add_widget(widget)
		return widget

	def add_floatSpinbox(self, name="", label="", tooltip="",
	                     value=0, min=None, max=None):
		widget = NodeFloatSpinbox(self, name, label, value, min, max)
		widget.setToolTip(tooltip)
		self.add_widget(widget)
		return widget
	...
	def delete(self):
		print "deleting all at view level"
		for port, text in self._input_items.items():
			port.delete()
		for port, text in self._output_items.items():
			port.delete()
	...
	def delete_io(self):
		print "deleting io at view level"
		for port, text in self._input_items.items():
			port.delete()
