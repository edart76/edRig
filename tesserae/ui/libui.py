...
class BaseWindow(QtWidgets.QMainWindow):
    ...
    def _setup_ui(self):
        ...
        if not os.name == 'nt':
            self.setWindowFlags(QtCore.Tool)
...
class BackdropContext(ContextWidget):
    ...
    def __init__(self, parent):
        ...
        self._color_dialog.setOptions(QtWidgets.QColorDialog.ShowAlphaChannel)
    ...
    def setup_ui(self):
        ...
        group = QtWidgets.QGroupBox("Edit Backdrop")
        ...
        description_field = QtWidgets.QTextEdit("test text")
    ...
    def on_filter_changed(self):
        ...
        if input_string != "":
            matched_items = self.tree_widget.findItems(input_string,
                                             (QtCore.MatchContains | QtCore.MatchRecursive | QtCore.MatchCaseSensitive))
...
class SearchField(ContextWidget):
    ...
    def _setup_completer(self, line_edit_widget):
        ...
        completer.setCaseSensitivity(QtCore.CaseInsensitive)
...
class RenameField(ContextWidget):
    ...
    def setup_ui(self):
        ...
        """
        self.mask = QtWidgets.QLineEdit(self)
        self.mask.setValidator( QtGui.QRegExpValidator(QtCore.QRegExp(self.validation_regex)))
        self.context = self.mask
        self.mask.setFocus()
...
class Backdrop(QtWidgets.QGraphicsRectItem):
    ...
    def __init__(self, name, **kwargs):
        ...
        # style
        self._bg_color = QtGui.QColor(*self._color)
        self._bg_border_color = QtGui.QColor(*self._border_color)

        self._bg_brush = QtGui.QBrush(self._bg_color, QtCore.SolidPattern)
        self._bg_pen = QtGui.QPen(QtCore.SolidLine)
        self._bg_pen.setJoinStyle(QtCore.RoundJoin)
        ...
        self._handle_brush = QtGui.QBrush(self._bg_color, QtCore.BDiagPattern)
        self._title_font = QtGui.QFont(self._font, self.title_font_size)
        self._title_font.setBold(True)

        self._description_font = QtGui.QFont(self._font, self.description_font_size)
    ...
    def setup_ui(self):
        ...
        self.title_bar = QtWidgets.QGraphicsRectItem(self._bounds[0],
                                                        self._bounds[1],
                                                        self._bounds[2],
        ...
        self.title = QtWidgets.QGraphicsTextItem(self.name, parent=self.title_bar)
        self.title.setFont(self._title_font)
        ...
        self.description = QtWidgets.QGraphicsTextItem(parent=self)
        self.description.setFont(self._description_font)
        ...
        self.handle = QtWidgets.QGraphicsRectItem(self._bounds[0] + self._bounds[2] - self._handle_size,
    ...
    def get_contained_items(self):
        ...
        """
        return self.scene().items(self.mapToScene(self.boundingRect()), mode=QtCore.IntersectsItemShape)
...
def set_application_palette(palette_filepath, application):
    ...
    if application:
        ...
        for role in roles:
            for group in groups:
                group_color = QtGui.QColor(palette_dict['{0}:{1}'.format(role, group)])
                group_group = getattr(QtGui.QPalette, group)
                group_role = getattr(QtGui.QPalette, role)
